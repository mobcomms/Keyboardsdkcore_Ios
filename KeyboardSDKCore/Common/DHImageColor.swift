...
//  DHUtils
//
//  Created by DHLee on 2021/03/02.
//
...
public enum DHImageColorsQuality: CGFloat {
    case lowest = 50 // 50px
    case low = 100 // 100px
    case high = 250 // 250px
...
fileprivate struct DHImageColorsCounter {
    let color: Double
    let count: Int
...
extension UIImage {
    ...
    private func resizeForDHImageColors(newSize: CGSize) -> UIImage? {
    ...
    public func getColors(quality: DHImageColorsQuality = .high, _ completion: @escaping (DHImageColors?) -> Void) {
    ...
    public func getColors(quality: DHImageColorsQuality = .high) -> DHImageColors? {
        ...
        guard let resizedImage = self.resizeForDHImageColors(newSize: scaleDownSize) else { return nil }
        ...
        let sortedColorComparator: Comparator = { (main, other) -> ComparisonResult in
            let m = main as! DHImageColorsCounter, o = other as! DHImageColorsCounter
            if m.count < o.count {
        ...
        while let K = enumerator.nextObject() as? Double {
            ...
            if threshold < C {
                sortedColors.add(DHImageColorsCounter(color: K, count: C))
            }
        }
        sortedColors.sort(comparator: sortedColorComparator)
        
        var proposedEdgeColor: DHImageColorsCounter
        if 0 < sortedColors.count {
            proposedEdgeColor = sortedColors.object(at: 0) as! DHImageColorsCounter
        } else {
            proposedEdgeColor = DHImageColorsCounter(color: 0, count: 1)
        }
        
        if proposedEdgeColor.color.isBlackOrWhite && 0 < sortedColors.count {
            for i in 1..<sortedColors.count {
                let nextProposedEdgeColor = sortedColors.object(at: i) as! DHImageColorsCounter
        ...
        for color in sortedColors {
            let color = (color as! DHImageColorsCounter).color
